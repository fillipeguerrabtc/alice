...
const COMMAND_PATTERNS: CommandPattern[] = [
  ...
  // GERAR SINAL
  {
    ...
    patterns: [
      ...
      /\b(oportunidades?\s+de\s+scalping)\b/i,
      /\b(setup(?:s)?\s+de\s+scalping)\b/i,
      /\b(verifique?|analise|avalie)\s+(?:as?\s+)?principais?\s+criptos?.*\bscalping\b/i,
      /\b(oportunidades?|setup(?:s)?|estrat[eé]gias?)\s+de\s+([a-z0-9-]+(?:\s+[a-z0-9-]+){0,3})\b.*\b(trading|trade|cripto|crypto|forex|futures?|futuros?|acoes|ações|stocks?|btc|eth|sol|xrp|bnb)\b/i,
      /\b(trading|trade|cripto|crypto|forex|futures?|futuros?|acoes|ações|stocks?|btc|eth|sol|xrp|bnb)\b.*\b(oportunidades?|setup(?:s)?|estrat[eé]gias?)\s+de\s+([a-z0-9-]+(?:\s+[a-z0-9-]+){0,3})\b/i,
