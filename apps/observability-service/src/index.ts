...
app.get('/api/observability/metrics/services', requireObservabilityRead, async (_req: Request, res: Response) => {
  try {
    const [
      ...
      queryPrometheus('up{job="alice-services"}'),
      queryPrometheus('avg by (service) (time() - process_start_time_seconds{job="alice-services"})'),
      queryPrometheus('sum by (service) (rate(alice_http_requests_total{job="alice-services"}[5m])) * 60'),
      queryPrometheus('sum by (service) (rate(alice_http_request_duration_seconds_sum{job="alice-services"}[5m])) / sum by (service) (rate(alice_http_request_duration_seconds_count{job="alice-services"}[5m]))'),
    ]);
...
app.get('/api/observability/metrics/circuit-breakers', requireObservabilityRead, async (_req: Request, res: Response) => {
  try {
    const [stateResult, failuresResult, successesResult] = await Promise.all([
      ...
      queryPrometheus('sum by (service, name) (rate(alice_circuit_breaker_failures_total{job="alice-services"}[5m])) * 60'),
      queryPrometheus('sum by (service, name) (rate(alice_circuit_breaker_successes_total{job="alice-services"}[5m])) * 60'),
    ]);
...
app.get('/api/observability/metrics/integrations', requireObservabilityRead, async (_req: Request, res: Response) => {
  try {
    const [configuredResult, operationalResult] = await Promise.all([
      ...
      queryPrometheus('alice_integrations_operational'),
    ]);
...
app.get('/api/observability/metrics/sla', requireObservabilityRead, async (req: Request, res: Response) => {
  ...
  try {
    const [
      ...
      queryPrometheus(`alice_sla_breached_total{tenant_id="${tenantId}"}`),
      queryPrometheus(`alice_sla_at_risk_total{tenant_id="${tenantId}"}`),
      queryPrometheus(`alice_sla_on_track_total{tenant_id="${tenantId}"}`),
      queryPrometheus(`alice_sla_avg_first_response_seconds{tenant_id="${tenantId}"}`),
      queryPrometheus(`alice_sla_avg_resolution_seconds{tenant_id="${tenantId}"}`),
    ]);
