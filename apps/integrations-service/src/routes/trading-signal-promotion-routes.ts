...
export function registerTradingSignalPromotionRoutes(
  ...
  const readPromotionPaths = [
    '/api/integrations/trading/signals/:id/promotion-path',
    '/api/integrations/trading/signals/:id/promotion', // Compatibilidade para consumers legados.
  ] as const;
  for (const routePath of readPromotionPaths) {
    app.get(routePath, requirePermission('integrations:trading:read'), async (req: Request, res: Response) => {
      try {
        ...
        logger.error({ error: errorMessage, routePath }, 'Erro ao consultar promotion path do sinal');
        res.status(500).json({ error: errorMessage });
      }
    });
  }

  const promoteEligibilityPaths = [
    '/api/integrations/trading/signals/:id/promote-real-eligibility',
    '/api/integrations/trading/signals/:id/promote-live-eligibility', // Compatibilidade para consumers legados.
  ] as const;
  for (const routePath of promoteEligibilityPaths) {
    app.post(routePath, requirePermission('integrations:trading:write'), async (req: Request, res: Response) => {
      try {
        const authContext = getTradingAuthContext(req);
        if (!authContext) {
          res.status(401).json({ error: 'Autenticação necessária' });
        ...
        if (deps.sendKucoinErrorResponse(res, error)) return;
        const errorMessage = error instanceof Error ? error.message : 'Erro desconhecido';
        logger.error({ error: errorMessage, routePath }, 'Erro ao promover sinal para real eligibility');
        res.status(500).json({ error: errorMessage });
      }
    });
  }
