      ...
      return tx
        ...
        .values(
          chunking.chunks.map((chunkContent, index) => ({
            documentId: document.id,
            tenantId: document.tenantId ?? job.tenantId,
            ownerUserId: document.ownerUserId ?? null,
            ownerGroupId: document.ownerGroupId ?? null,
            scopeType: document.scopeType ?? 'tenant',
            visibility: document.visibility ?? 'private',
            sensitivityLabel: document.sensitivityLabel ?? 'confidential',
    ...
    await upsertPoints(
      ...
      insertedChunks.map((chunk, index) => ({
        ...
        payload: {
          ...
          scopeKey: `tenant:${job.tenantId}:resource:${document.id}`,
          documentId: document.id,
          chunkId: chunk.id,
