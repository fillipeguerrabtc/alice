...
- **Training Queue Stall Root Cause + Fix (06/03/2026):** investigação completa em produção confirmou que o run não ficava apenas "na fila"; o `gpu-manager` falhava ao subir `gpu-trainer` com `docker compose` por `permission denied` no `--env-file /opt/alice/compose/.env.prod` (container roda como usuário `node`). Correção cirúrgica no `gpu-orchestrator`: manter tentativa principal com `--env-file` e aplicar fallback automático sem `--env-file` somente quando o erro for de permissão nesse arquivo, reutilizando as variáveis já carregadas no processo do `gpu-manager` via `env_file`. Inclui teste unitário de regressão para garantir retry controlado e evitar fallback em erros não relacionados.
