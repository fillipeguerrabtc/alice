...
- **Hardening cirúrgico de integrações externas + GPU/LLM runtime (10/03/2026):** `apps/integrations-service/src/kucoinClient.ts`, `apps/integrations-service/src/kucoinRequest.ts`, `apps/integrations-service/src/kucoinUnifiedWebSocket.ts`, `apps/integrations-service/src/wiseClient.ts`, `apps/integrations-service/src/wiseService.ts`, `apps/integrations-service/src/index.ts`, `apps/gpu-manager-service/src/index.ts` e `apps/llm-gateway-service/src/index.ts` passaram a consumir parsing tipado de `@alice/config` para URLs, secrets e parâmetros de timeout/VRAM/admission, com redução de leitura direta de `process.env`, validação determinística de bootstrap e manutenção das estratégias já existentes de retry/circuit breaker/governance sem alterar regra de negócio.
- **Hardening cirúrgico de runtime em Chat/RAG/Training (10/03/2026):** `apps/chat-service/src/rag-client.ts`, `apps/chat-service/src/lora-adapter-resolver.ts`, `apps/chat-service/src/response-cache.ts`, `apps/rag-service/src/index.ts`, `apps/rag-service/src/workers/web-crawl-worker.ts` e `apps/training-service/src/index.ts` reduziram leituras diretas de `process.env` no escopo crítico, passaram a consumir helpers tipados de `@alice/config`, removeram hardcode operacional de domínio no web-crawler, reforçaram fail-fast de `GPU_MANAGER_URL` em produção no training-service e adicionaram validação de payload em borda no endpoint `POST /api/rag/classify`, preservando invariantes assíncronas de filas/idempotência/jobs.
- **Hardening cirúrgico de gateway/auth em runtime config (10/03/2026):** `apps/api-gateway/src/index.ts` e `apps/auth-service/src/index.ts` passaram a resolver `CORS`, `BASE_URL`, domínio de tenant, callbacks OAuth e URLs internas via configuração tipada central (`@alice/config`), com fail-fast em produção para lacunas críticas e fallback restrito a desenvolvimento; hardcodes operacionais (`yesyoudeserve.duckdns.org`, fallback fixo de `GRAFANA_URL`) foram removidos do app code.
- **Hardening de lifecycle para fail-fast em entrypoints (10/03/2026):** terminação forçada de processo foi removida de utilitários compartilhados (`packages/shared-utils/src/session-auth.ts`, `packages/shared-utils/src/gpu-client.ts`, `packages/shared-utils/src/rbac/middleware.ts`, `packages/shared-utils/src/shutdown-manager.ts`) e de módulos de suporte de serviço (`apps/auth-service/src/oidc/configuration.ts`, `apps/chat-service/src/runtime-config.ts`, `apps/integrations-service/src/integrations-bootstrap-service.ts`, `apps/gpu-manager-service/src/gpu-client.ts`, `apps/rag-service/src/image-processor.ts`, `apps/rag-service/src/audio-processor.ts`); decisões de `process.exit(1)` foram consolidadas nos entrypoints (`apps/*/src/index.ts`) com logs estruturados e rastreabilidade de falha.
