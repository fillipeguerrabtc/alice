...
> **NOTA (04/01/2026):** **CORREÇÃO ENTERPRISE - Loop de Colisão para Primeiro Release**: Corrigido bug no `release.yml` onde o loop de verificação de colisão de tags (que incrementa PATCH até encontrar versão disponível) estava apenas dentro do branch `else` (versão calculada). **Causa**: Quando `PREVIOUS_TAG` estava vazio (primeiro release), o código definia `NEXT_VERSION="v1.0.0"` diretamente e pulava o loop de colisão. **Impacto**: Se `v1.0.0` já existisse (criada manualmente, em outra branch, ou de release anterior falhado), o workflow falhava ao tentar criar a tag. **Solução**: Loop de colisão movido para FORA do if/else, aplicando-se a ambos os cenários (primeiro release e versões incrementadas). Agora define `MAJOR=1, MINOR=0, PATCH=0` e permite incremento automático se necessário.
