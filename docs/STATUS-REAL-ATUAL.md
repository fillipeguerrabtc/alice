...
| TTS runtime | Parâmetros via `MEDIA_PARAMS` JSON (text, voice, lang) com fallback para env vars; **idioma default: `pt`** (Regra 13 - PT-BR primário); prioridade: `MEDIA_PARAMS.lang` > `TTS_LANG` env > default | `docker/tts/serve.py`, `apps/rag-service/src/workers/media-worker.ts` |
