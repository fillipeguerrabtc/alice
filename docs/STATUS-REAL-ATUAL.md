...
> **Versão:** 3.20 - Cache pnpm estável no deploy
...
>
> **NOTA (12/12/2025):** Cache pnpm no deploy-prod: removida flag `cache: pnpm` do `actions/setup-node` (erro 400 intermitente) e adicionado cache explícito com `actions/cache@v4` usando store-dir fixo (`$HOME/.pnpm-store`) e chave baseada em OS + versão do pnpm + hash do `pnpm-lock.yaml`.
