/**
 * Schema Principal - Alice Enterprise Platform
 * 
 * Estado da modularização nesta fase (17/03/2026):
 * - `schema.ts` continua sendo a surface pública canônica de compatibilidade.
 * - Contratos puros (custom types + schemas JSONB/Zod) foram extraídos para `./schema/`.
 * - Tabelas Drizzle, relations, insert schemas e inferências permanecem centralizados aqui.
 * 
 * Para modificações no schema:
 * 1. Edite ESTE arquivo (schema.ts)
 * 2. Gere migrations com `pnpm drizzle-kit generate`
 * 3. Se o ajuste tocar contratos puros reutilizáveis, sincronize os módulos em `./schema/`
 * 
 * Multi-tenancy:
...
import {
  ...
  trainingVector1024,
  vector,
} from "./schema/custom-types.js";
import { DatasetSplitPolicySchema } from "./schema/jsonb-contracts.js";
import type {
  ...
  LearningTaskParametros,
  LearningTaskResultado,
  LlmConfigAvancada,
  MessageAnexos,
  MessageMetadata,
  ModelVersionMetrics,
  ...
  UserPreferencias,
  WebhookPayload,
} from "./schema/jsonb-contracts.js";

export * from "./schema/jsonb-contracts.js";
...
export const TradingLoraHyperparamsSchema = z.object({
  ...
  batchSize: z.number().default(2),
  epochs: z.number().default(3),
  warmupSteps: z.number().default(0),
  gradientAccumulationSteps: z.number().int().min(1).default(2),
  ...
  maxSeqLen: z.number().int().min(256).max(32768).default(1536),
});
