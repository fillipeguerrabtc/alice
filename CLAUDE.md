...
*Versão: 4.30 - 26 de Dezembro de 2025*
...
*Bug Fix gpu-manager-service Build CI Failure (26/12/2025): CAUSA RAIZ CORRIGIDA - O gpu-manager-service usava `build`: `tsc` que gerava `dist/index.js`, mas o CI workflow esperava `dist/bundle.js` para todos os servicos exceto api-gateway e observability-service. Corrigido: package.json atualizado para usar `build`: `node ../../scripts/build-service.mjs gpu-manager-service` (padrao enterprise dos outros microsservicos), `main` alterado para `dist/bundle.js`, `esbuild` adicionado como devDependency, scripts padronizados (`typecheck`, `lint`, `build:tsc`) adicionados para consistencia. Build agora gera corretamente `dist/bundle.js` (93.6kb) e CI workflow passa. Implementacao 100% enterprise-grade (Regra 6 - padrao consistente entre microsservicos, sem excecoes).*
