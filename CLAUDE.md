...
*Versão: 4.27 - 26 de Dezembro de 2025*
...
*Bug Fix Root Cause: WebM Content-Type Mismatch (23/12/2025): CAUSA RAIZ CORRIGIDA - Arquivos `.webm` eram sempre servidos com `Content-Type: audio/webm`, mas WebM pode ser tanto áudio quanto vídeo. Isso quebrava o serviço de arquivos legados que são vídeos, causando problemas de playback e comportamento inesperado em clientes que verificam o MIME type. Corrigido: O endpoint `/api/media/files/:tenantId/:mediaType/:filename` agora busca o MIME type original do banco de dados (tabela `mediaUploads.mimeType`) antes de determinar o Content-Type. Se o arquivo não estiver no banco (arquivo legado sem registro), usa `video/webm` como fallback (que funciona para ambos áudio e vídeo, pois é um superconjunto). Isso garante que arquivos novos usam o MIME type correto do banco, arquivos legados de vídeo WebM são servidos como `video/webm` (correto), e arquivos legados de áudio WebM também funcionam com `video/webm`. Implementação 100% enterprise-grade (Regra 6 - Content-Type correto baseado em dados reais, sem suposições, fallback seguro para arquivos legados).*
