...
*Bug Fix FILE_LIMITS Type Safety and Undefined Access (23/12/2025): O objeto `FILE_LIMITS` era definido como `as const` sem tipo explícito `Record<MediaType, number>`, permitindo que TypeScript não garantisse que todas as chaves de `MediaType` existissem. Após validar que `mediaType` não é null, o código acessava `FILE_LIMITS[mediaType]` sem verificar se a chave existe. Se um tipo de mídia inesperado passasse pela validação, ou se `MediaType` não correspondesse às chaves de `FILE_LIMITS`, isso resultava em acesso a `undefined` e divisão por `undefined` (ex: `limit / (1024 * 1024)`), causando valores `NaN` em mensagens de erro. Corrigido: adicionado tipo explícito `Record<MediaType, number>` ou `Record<'image' | 'audio', number>` em todos os arquivos (`Training.tsx`, `use-websocket-chat.ts`, `Chat/components/types.ts`), e adicionada verificação defensiva para garantir que `limit` não seja `undefined` antes de usar em cálculos. Implementação 100% enterprise-grade (Regra 6 - type safety completo, prevenção de NaN, validação defensiva).*
