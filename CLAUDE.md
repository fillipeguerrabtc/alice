...
*Bug Fix ERPNext Workers Healthchecks (22/12/2025): ERPNext workers tinham healthchecks que pingavam erpnext-backend externo. Corrigido: healthchecks alterados para verificar processo local (`pgrep -f 'bench worker'` ou `bench schedule`) ao invés de pingar backend. Workers NÃO dependem do backend - só de Redis/MariaDB para processar jobs.*
