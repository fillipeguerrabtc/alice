...
*Bug Fix ERPNext Workers Dependency Chain (22/12/2025): ERPNext workers (scheduler, worker-default, worker-short, worker-long e suas réplicas -2) tinham healthchecks que pingavam erpnext-backend, mas NÃO dependiam dele! Isso causava workers serem marcados unhealthy antes do backend iniciar. Corrigido: adicionada dependência `erpnext-backend: service_healthy` em 7 serviços e healthchecks alterados para verificar processo local (`pgrep -f 'bench worker'`) ao invés de pingar backend externo.*
