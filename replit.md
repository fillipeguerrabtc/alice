...
-   **Express Hardening Module (Fase 6 OWASP)**: Módulo centralizado `packages/shared-utils/src/express-hardening.ts` aplicado em 5 microsserviços (auth, chat, integrations, rag, training). Inclui: (1) `createSecurityMiddleware` com Helmet 7.x, CSP strict em produção (sem unsafe-inline para scripts), HSTS preload, X-Frame-Options deny; (2) `createRateLimiter` multi-tenant com ipKeyGenerator para IPv6 subnet handling (express-rate-limit 8.x), leitura de tenant via header x-tenant-id antes do auth middleware; (3) `createErrorHandler` global que nunca vaza stack traces em produção (verificação estrita de NODE_ENV); (4) `createNotFoundHandler` com identificação de serviço; (5) `asyncHandler` wrapper para rotas async. Middleware ordering padronizado: helmet → correlation → compression → cors → rateLimit → body parsers → routes → notFound → errorHandler.
