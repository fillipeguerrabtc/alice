...
if [ ! -d "$PGDATA" ]; then
  ...
  echo "   sudo chown 70:70 /opt/alice/data/postgres"
...
if [ ! -w "$PGDATA" ]; then
  ...
  echo "============================================="
  echo ""
  echo "   # Corrigir ownership para UID 70 (postgres Alpine)"
  echo "   sudo chown -R 70:70 /opt/alice/data/postgres"
  echo ""
  ...
  echo "   - CLAUDE.md Regra 6 (Enterprise-grade)"
  echo "   - DEPLOYMENT.md Seção 8.2 (Permissões por serviço)"
  echo "   - PostgreSQL Alpine espera UID 70 como owner do PGDATA"
  echo ""
  exit 1
fi
